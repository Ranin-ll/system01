...
public interface ExamRuleMapper {
    ...
    /** 本部门题池里已有的知识点及题量（供「从题池导入知识点」） */
    List<Map<String, Object>> selectDeptKnowledgePoints(@Param("deptId") Long deptId);

    /** 本部门题池按题型的可用题量（配置页「题池概览」用） */
    Map<String, Object> selectDeptAvailable(@Param("deptId") Long deptId);
    ...
    /**
     * 按部门题池 + 知识点 + 题型随机抽题。
     * point 为 null/空 时不限知识点（退化为「本部门题池按题型取题」）。
     */
    List<Question> selectQuestionsByPoint(@Param("deptId") Long deptId,
     ...
     * 只返回 PRE_TRAINEE（预备实习生）且账号正常的用户。
     */
